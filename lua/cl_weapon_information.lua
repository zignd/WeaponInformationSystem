	...
	if IsValid( ent ) then
		...
		if ent:IsWeapon() and ent:IsScripted() then
			...
			if string.len(wpninfo.infos.name) > 14 then
				panelwidth = 600 + string.len(wpninfo.infos.name) * 10
			else
				panelwidth = 600
			end
			...
			position = ent:GetPos() + Vector( 0, 0, 37 + math.sin( CurTime() * 2 ) * 5 )	
		...
		elseif ent.Type and ent.AmmoType and ent.Type == "anim" then
			...
			position = ent:GetPos() + Vector( 0, 0, 20 + math.sin( CurTime() * 2 ) * 5 )
